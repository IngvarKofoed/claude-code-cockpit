...
86. The usage-bar subscription path (`handleInternalUsage`) now logs to daemon.log (metadata only) (v0.33.0):
...
87. daemon.log now self-bounds (v0.33.0) — rotated to `daemon.log.1` once it passes ~5MB (checked at boot, then hourly),
...
88. The usage auto-pilot now watches BOTH rate-limit windows, each with its own threshold (v0.34.0):
...
194. The Live view's "Repository name" sort now orders sessions WITHIN a repo by SESSION NAME
...
207. Every dollar figure now renders with exactly TWO decimals — the dashboard's `fmtCost`
