...
- **Which sessions belong together.** Sessions can be gathered into named **groups** — a two-session spec/implement pair, say — each rendered as a bounded box that grows either down (a stacked column) or across (a band), holding its cards in an order you set — and the boxes themselves ordered the same way, from the group header. A group identifies the *terminal seat* rather than the session, so clearing a session doesn't drop it out of its group.
...
1. **Live session overview.** Per active session: repository (git root name + full path), branch, status (`running` / `waiting-for-input` / `idle` / `paused` / `error`), the elapsed time of the current prompt (ticking live), and session age. A session given a colour with Claude Code's `/color` carries that colour as a **dot beside its name** — the same cue already on that session's prompt bar, so a card can be matched to the terminal it belongs to at a glance (and cleared again with `/color default`). A session running in a macOS Terminal window, or in any Windows terminal, can also be **jumped to** — one click raises the window it is running in, so a card that says "waiting for input" leads straight to the place you answer it. (On Windows Terminal that means the right window but not the right tab; there is no way to select a tab by process.)
...
3. **Per-repository accounting.** Total active time, prompt count, session count (counting only sessions that spent tokens — an opened-but-never-worked session is excluded), token totals (input / output / cache), and an **estimated dollar cost**, with a time-range filter. *Active time* is engaged wall-clock — time a session spent working a turn or running a background workflow — and deliberately **excludes** time spent waiting on you (a permission prompt, or a question/plan the agent is holding for your answer) or sitting idle, so it reflects real work rather than elapsed clock. Usage is **attributed per subscription**, enabling per-subscription stats and trends: a closed turn is filed under the account signed in when it closed, so a mid-day account switch doesn't keep billing a long-running session to the account it merely started on. The live usage bars likewise track the account signed in *now*, accepting only the freshest reading and reading "account switched — awaiting update" rather than showing one account's numbers under another's name.
...
5. **Session directory.** A newest-first, paginated list of every Claude Code session still on disk **that spent tokens** — read straight from the transcript files, so it covers pre-cockpit and already-ended sessions the accounting store no longer holds. Sessions that spent **0 tokens (and so 0 cost)** — opened but never worked — are filtered out of this list (and out of every session count elsewhere); a session whose transcript can't be read is *unknown*, not zero, so it stays listed with tokens shown as "—". Each row shows the session's name — the one the user set with `/rename`, or the AI-generated title when they haven't renamed it — its repository, when it was last active, and its token/cost totals, with live sessions badged active. Because it reads Claude Code's transcripts (not the cockpit's store), it follows Claude Code's retention — a repo cleared from the accounting views still lists its (token-bearing) sessions here. The verbatim last prompt is never shown; only the derived or deliberately user-typed name.
...
7. **Pause gate.** An optional control-file-based "all-sessions freeze" — one toggle button or slash command instantly blocks every Claude Code session's tool execution via a separate `PreToolUse` gate hook, and resumes all at once when flipped back. The gate is **on by default** (`pauseGateEnabled` config; a fresh install ships with it enabled — a user can still turn it off, and an explicit off is never overridden), fail-open (missing / garbage control file runs tools), and includes an auto-pilot (also on by default at a 90% threshold): pause when 5-hour usage crosses the threshold, resume once usage falls a hysteresis deadband back below it (so a rolling window wobbling across the threshold can't flap the gate). Pause/resume are recorded in the event log and shown on the dashboard and statusline. The dashboard also surfaces, per session, when a tool call has actually been frozen (with a 'Paused — parked' badge and frozen timer), and fires an optional OS notification when all sessions are at rest — the walk-away signal. *Privacy note: pause events carry no message content, only metadata.*
