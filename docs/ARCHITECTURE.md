...
- **Windows window** (`pid:1234`) — walk the `Win32_Process` parent chain to the first ancestor owning a window, then restore-if-minimized and `SetForegroundWindow`. Retained only as a fallback for hosts that *are* in the session's process tree, chiefly a VS Code integrated terminal.

**Why the Windows path is title-keyed rather than process-keyed.** On a default Windows 11 setup the terminal window is simply not reachable from the session's process tree — measured live: `claude.exe → cmd.exe → explorer.exe`, every entry reporting `MainWindowHandle = 0`, because the default-terminal handoff reparents the console to `WindowsTerminal.exe` under `svchost.exe`. The `conhost.exe` child is a ConPTY shim owning no window. **Do not re-attempt this via parent/child traversal.** UI Automation is the only interface that both enumerates background tabs and can select one, and it exposes no link back to the hosted shell — every `TabItem` reports the `WindowsTerminal.exe` pid — so the tab **name** is the sole available join key. Claude Code writes the session name into it (`⠂ testing`, the leading glyph animating per render, hence the normalization that strips it).
