...
## Rate-limit usage bars (subscription-filtered) + the per-session context gauge

The statusline forwarder's `POST /internal/usage` carries two independent things at two different **scopes**: `rate_limits` (account-wide → the Live ribbon's 5h and weekly bars) and `context_window` (one session's context fill → that card's gauge). `handleInternalUsage` applies the context reading **first and unconditionally**, ahead of every rate-limit guard below, precisely because the scopes differ: a push from a stale subscription carries the wrong account-wide numbers but still reports its *own* session's true context fill, and an API-key session carries no `rate_limits` at all yet still has a context window. Either half alone is a valid push.
