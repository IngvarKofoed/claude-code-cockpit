...
**The weekly bar's burn rate, and the sample buffer behind it.** Both of that bar's readouts — the multiplier and the projected-limit clause — reduce to one quantity, a burn rate, which by default is the average *since the window opened*. Over seven days that is dominated by the hours the user was asleep. `usageWeeklyLookbackHours` (see *Configuration*) switches it to a **sliding lookback** instead, which needs a short history of the weekly percentage that the single overwritten `rateLimitUsage` snapshot cannot provide. The daemon therefore keeps `usageSamples` — `{ t, pct, sub }`, appended **change-only** (the statusline posts on every render), snapshot-persisted, and pruned to 24h **except the newest entry per subscription beyond that horizon**, which is kept as the anchor a lookback measures against. Dropping that anchor would strand the case it serves: a flat stretch longer than the horizon would lose the only evidence the value is flat. Samples are **subscription-filtered, never cleared on a switch** (`currentSubscription` flips back and forth in practice, and a delta spanning a flip reads tens of points negative); a drop of ≥10 points discards that subscription's history as a window reset rather than a rolling window's gradual decline. `buildStatePayload` ships only the slice inside the configured lookback plus that one out-of-window anchor — which rides along deliberately, because the browser re-derives the rate every second and the sample it needs at `now − lookback` moves *forward* through the array as its clock advances past the frame's. With the default `0` the field is omitted entirely.

Under a sliding lookback **only**, the multiplier's reference also changes, to what the *remaining* budget sustains — `rate / ((100 − used) / timeToReset)` — so `1.0×` means "on track to finish this window at 100%", and since `m > 1 ⟺ the projection lands before the reset` the two readouts agree on the **raw** ratio. They can still *look* like they disagree, because the multiplier is rounded for display and the clause is not: a true 1.02× shows as a muted `1.0×` beside an amber `limit in ~3h`. That is entry 159's accepted trade — precision, not contradiction — and the reframing does not remove it, since gating the clause on the rounded ratio would hide a real multi-hour shortfall whenever it rounded down. The 5h bar and `Week` keep `usedFrac / elapsedFrac` untouched; the accepted cost is that `1.0×` means two different things depending on the bar, disambiguated by the tooltip. Because `used_percentage` arrives as an **integer**, any delta carries ±1 point of quantization error whatever the span, so below 2 points of movement the readouts state the bound they can prove (`< 2%/6h`) rather than a number that could be off by half, and with no anchor old enough they read `measuring…` — the two are kept distinct because one resolves itself and the other is a real statement about low usage. Precision is worst near the cap, where one point is a large share of what is left. **Known limit:** the rate divides by the nominal lookback, which is exact only while the buffer is gap-free — true by construction while the daemon runs, but a daemon that was **down while the percentage rose** attributes the whole gap-sized delta to the lookback and overstates the rate until a post-restart sample crosses the cutoff.
...
**`usageWeeklyLookbackHours`** (`0 | 2 | 3 | 6 | 12 | 24`, default `0`) selects what the **weekly** bar's burn-rate readouts measure over. `0` is the whole window — the average since it opened, and the pre-feature behaviour — so an upgrade changes nothing until the dropdown is touched. A positive value switches that bar to a sliding lookback; see *Rate-limit usage bars* above. Validated against exactly that set (a numeric string coerces like `port`, but `null`/`''`/a boolean must not: `Number()` maps all three to `0`, which is a real option here and would turn a malformed value into a silent preference).
