...
function newSession(event) {
  return {
    ...
    // Session name — the user's /rename (`custom-title`) when set, else the generated
    // `ai-title` — daemon-filled from readUsage on each token read; null until one exists
...
function accumulateActiveFromEvents(rollup, events) {
  ...
  for (const ev of events) {
    ...
    // Per-repo subagent count BROKEN OUT BY agent_type, same unconditional event-derived
    // pattern as byTool/subagents. Only SubagentStart events that carry an agent_type count.
